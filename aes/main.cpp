...
void mix_columns(unsigned char* state) {
    ...
    {
        tmp[i] = (unsigned char)(mul2[state[i + 0]] ^ mul3[state[i + 1]] ^ state[i + 2] ^ state[i + 3]);
        tmp[i + 1] = (unsigned char)(state[i + 0] ^ mul2[state[i + 1]] ^ mul3[state[i + 2]] ^ state[i + 3]);
        tmp[i + 2] = (unsigned char)(state[i + 0] ^ state[i + 1] ^ mul2[state[i + 2]] ^ mul3[state[i + 3]]);
        tmp[i + 3] = (unsigned char)(mul3[state[i + 0]] ^ state[i + 1] ^ state[i + 2] ^ mul2[state[i + 3]]);
    }
...
{    FILE* out = fopen("output.txt", "wb+");
    ...
    if (fseek(inp, 0L, SEEK_END))
        ...
        size = ftell(inp);
    fclose(inp);
    FILE* in = fopen("input1.txt", "rb");
    for (int counter = 0; counter < size / 16; counter++) {
        for (int k = 0; k < 16; ++k) {
            msg[k] = fgetc(inp);
           // cout << msg[k];
        }
